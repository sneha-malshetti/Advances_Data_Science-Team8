...
with open('C:\\Users\\megha\\Desktop\\Notebook\\New folder\\config.json') as data_file:    
    data = json.load(data_file)
    
with open('C:\\Users\\megha\\Desktop\\Notebook\\New folder\\initconfig.json') as data_file1:    
...
fname = state1+"_"+datestr+"_WBAN_54779"+".csv"
...
#download on local directory init data    
    ...
    initfullmerge2.to_csv(initialfilename,sep=',')
    # log file save event
...
if possiblekey==None:
        ...
        prevdata= pd.read_csv(lastchangedfile)
        
        #Merge previous and today
        
        dailymerge=pd.concat([initfile1,initfile2], axis=0).drop_duplicates().reset_index(drop=True)
